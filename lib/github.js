...
const PREVIEW_DATA_RE = /<!--\s*admin-preview-data:([A-Za-z0-9+/=]+)\s*-->/i;
...
export function getPreviewEntries(prOrBody) {
  const body = typeof prOrBody === 'string' ? prOrBody : prOrBody?.body;
  const match = String(body || '').match(PREVIEW_DATA_RE);
  if (!match) return getFallbackPreviewEntries(body);
  try {
    const parsed = JSON.parse(Buffer.from(match[1], 'base64').toString('utf8'));
    return Array.isArray(parsed) ? parsed : [];
  } catch {
    return getFallbackPreviewEntries(body);
  }
}

function withPreviewEntries(body, entries) {
  const cleanBody = String(body || '').replace(PREVIEW_DATA_RE, '').trim();
  const encoded = Buffer.from(JSON.stringify(entries), 'utf8').toString('base64');
  return [`<!-- admin-preview-data:${encoded} -->`, cleanBody].filter(Boolean).join('\n\n');
}

function getFallbackPreviewEntries(body) {
  const entries = [];
...
function previewEntryFor(section, payload) {
  return {
    section,
    createdAt: new Date().toISOString(),
    content: section === 'publications' ? (payload.publication || {}) : (payload.entry || {}),
  };
}
...
export async function ensureOpenPr(token, targetRepo, writeRepo, baseBranch, draftBranch, section, payload) {
  ...
  const previewEntry = previewEntryFor(section, payload);
  if (existing) {
    const bodyWithPreview = withPreviewEntries(
      `${existing.body || ''}\n${bullet}`.trim(),
      [...getPreviewEntries(existing.body), previewEntry],
    );
    const newBody = withReviewStatus(bodyWithPreview, REVIEW_STATUS_PENDING);
  ...
  const bodyWithoutStatus = withPreviewEntries([
    'Pending admin edits queued by the admin app. Review the admin preview, then merge to publish.',
    '',
    '### Queued edits',
    bullet,
  ].join('\n'), [previewEntry]);
  const body = withReviewStatus(bodyWithoutStatus, REVIEW_STATUS_PENDING);
