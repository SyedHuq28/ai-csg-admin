...
export function getConfig() {
  ...
  return { githubToken, repo, baseBranch, draftBranch, previewTemplate };
}
...
export async function getOpenAdminPr(token, repo, draftBranch) {
  const owner = repo.split('/')[0];
  const res = await ghFetch(
    token,
    `/repos/${repo}/pulls?head=${encodeURIComponent(`${owner}:${draftBranch}`)}&state=open`,
  );
...
export async function ensureOpenPr(token, repo, baseBranch, draftBranch, section, payload) {
  const existing = await getOpenAdminPr(token, repo, draftBranch);
  const bullet = bulletFor(section, payload);
  if (existing) {
    const newBody = `${existing.body || ''}\n${bullet}`.trim();
    await ghFetch(token, `/repos/${repo}/pulls/${existing.number}`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
  ...
  const createRes = await ghFetch(token, `/repos/${repo}/pulls`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
